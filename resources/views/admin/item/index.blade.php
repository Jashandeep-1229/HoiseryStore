                ...
                <div class="card">
                    <div class="card-body">
                        <div  id="basic-2_wrapper" class="dataTables_wrapper px-2" onchange="get_datatable()">
                            ...
                            <div class="dataTables_filter">
                                <label>Season:
                                    <select class="form-control" name="season_filter" id="season_filter">
...
@section('script')
    <script>
        ...
        function get_datatable(page){
            ...
            var season_filter = $('#season_filter').val();
            var page = page ?? 1;
            $.get('{{ route("item.datatable") }}?page='+page+'&value='+value+'&search='+search+'', { _token: "{{csrf_token() }}",season_filter:season_filter}, function(data){
        ...
        function change_status(id){
            var url = "{{route('item.change_status',":id")}}";
            url = url.replace(':id',id);
            $.get(url, function(data){
                if(data.result == 1){
