        ...
        <div class="row">
            <div class="col-12">
                <div class="card" id="add_brand">
                    <form action="{{route('brand.store')}}" method="POST" id="" class="modal-content" enctype="multipart/form-data">
                        @csrf
...
@section('script')
    <script>
        ...
        function get_datatable(page){
            ...
            var page = page ?? 1;
            $.get('{{ route("brand.datatable") }}?page='+page+'&value='+value+'&search='+search+'', { _token: "{{csrf_token() }}"}, function(data){
        ...
        function change_status(id){
            var url = "{{route('brand.change_status',":id")}}";
            url = url.replace(':id',id);
            $.get(url, function(data){
                if(data.result == 1){
