<div class="dt-ext table-responsive">
    <table class="display table-striped table-hover" id="basic-test">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $income->firstItem() + $key }}</td>
                <td>{{ date('d M,Y',strtotime($item->date)) }}</td>
                <td>{{ $item->account->name ?? 'N/A' }}</td>
                <td>{{ $item->remarks ?? '' }}</td>
                <td class="text-success">{{ formatIndianNumber($item->amount) ?? '' }}</td>
                ...
                <td>
                    ...
                    @if (auth()->user()->role_as == 'Admin')
                        <a onclick="delete_income({{$item->id}})" class="btn btn-danger btn-sm  pointer p-1 f-14" data-toggle="tooltip" title="Delete">
