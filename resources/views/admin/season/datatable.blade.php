<div class="dt-ext table-responsive">
    <table class="display table-striped table-hover" id="basic-test">
        ...
        <tbody>
            @foreach ($season as $key => $item)
            <tr>
                <td>{{ $season->firstItem() + $key }}</td>
                <td>{{ $item->name ?? 'N/A' }}</td>
                <td>
                ...
                <td>
                    ...
                    @if (auth()->user()->role_as == 'Admin')
                        <a onclick="delete_season({{$item->id}})" class="btn btn-danger btn-sm  pointer p-1 f-14" data-toggle="tooltip" title="Delete">
