...
@section('breadcrumb-items')
    <li class="breadcrumb-item">Season</li>
@endsection
...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                <div class="card" id="add_season">
                    <form action="{{route('season.store')}}" method="POST" id="" class="modal-content" enctype="multipart/form-data">
                        @csrf
...
@section('script')
    <script>
        ...
        function get_datatable(page){
            ...
            var page = page ?? 1;
            $.get('{{ route("season.datatable") }}?page='+page+'&value='+value+'&search='+search+'', { _token: "{{csrf_token() }}"}, function(data){
        ...
        function edit_modal(id,key_value){
            var url = "{{route('season.edit_modal',":id")}}";
        ...
        function delete_season(id){
            ...
            .then((willDelete) => {
                if (willDelete) {
                    var url = "{{route('season.delete',":id")}}";
                    url = url.replace(':id',id);
        ...
        function change_status(id){
            var url = "{{route('season.change_status',":id")}}";
            url = url.replace(':id',id);
            $.get(url, function(data){
                if(data.result == 1){
