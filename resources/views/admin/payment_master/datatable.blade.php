<div class="dt-ext table-responsive">
    <table class="display table-striped table-hover" id="basic-test">
        ...
        <tbody>
            @foreach ($payment_master as $key => $item)
            <tr>
                <td>{{ $payment_master->firstItem() + $key }}</td>
                <td>{{ $item->name ?? 'N/A' }}</td>
                <td>
                ...
                <td>
                    ...
                    @if (auth()->user()->role_as == 'Admin')
                        <a onclick="delete_payment_master({{$item->id}})" class="btn btn-danger btn-sm  pointer p-1 f-14" data-toggle="tooltip" title="Delete">
...
<div class="mt-2">
    {{$payment_master->onEachSide(1)->links()}}
</div>